...
export const standardFaq: { q: string; a: string }[] = [
  {
    ...
    a: 'Nie — badanie onkopierwiastków zamawiasz online bez skierowania lekarskiego. Po złożeniu zamówienia na onkopierwiastki.pl zgłaszasz się do wybranej placówki referencyjnej na pobranie krwi. Nie musisz konsultować się wcześniej z lekarzem, choć wynik warto omówić ze specjalistą.',
  },
  {
...
export const cities: CityData[] = [
  {
    ...
    cityFaq: [
      { q: 'Gdzie zrobić onkopakiet we Wrocławiu?', a: 'Onkopakiet we Wrocławiu wykonasz w dwóch certyfikowanych placówkach referencyjnych: Dolnośląskie Centrum Medyczne DOLMED S.A. przy ul. Legnickiej 40 oraz Centrum Medyczne Polmed przy ul. Grabiszyńskiej 208. Obie placówki pobierają krew pełną zgodnie z protokołem laboratorium Innowacyjna Medycyna.' },
      ...
      { q: 'Czy w placówkach DOLMED i Polmed we Wrocławiu jest parking?', a: 'Dolnośląskie Centrum Medyczne DOLMED przy ul. Legnickiej 40 dysponuje parkingiem dla pacjentów i jest dobrze skomunikowane tramwajem. Centrum Medyczne Polmed przy ul. Grabiszyńskiej 208 również jest dostępne komunikacją miejską. Obie placówki działają w dni robocze.' },
      { q: 'Czy onkopakiet we Wrocławiu mogę wykonać w sobotę?', a: 'Obie placówki referencyjne we Wrocławiu — DOLMED i Polmed — działają w dni robocze. Sprawdź aktualne godziny pracy placówki na jej stronie internetowej lub zadzwoń pod numer telefonu podany przy placówce.' },
    ],
    seoText: `<h3>Badanie onkopierwiastków we Wrocławiu — innowacyjna profilaktyka nowotworowa</h3>
<p>Wrocław, stolica Dolnego Śląska z populacją przekraczającą 670 tysięcy mieszkańców, jest jednym z miast w Polsce, w którym można wykonać <strong>badanie onkopierwiastków</strong> w certyfikowanej placówce referencyjnej. <strong>Onkopakiet Wrocław</strong> stanowi nowatorskie podejście do profilaktyki nowotworowej, oparte na ponad dwudziestu latach badań naukowych prowadzonych przez zespół prof. dr. hab. n. med. Jana Lubińskiego z Pomorskiego Uniwersytetu Medycznego w Szczecinie.</p>

<p><strong>Onkopakiet</strong> dostępny we Wrocławiu obejmuje oznaczenie stężeń sześciu pierwiastków śladowych we krwi pełnej: <strong>selenu (Se), cynku (Zn), arsenu (As), miedzi (Cu), kadmu (Cd) i ołowiu (Pb)</strong>. Każdy z tych pierwiastków odgrywa istotną rolę w procesach biologicznych organizmu, a ich nieprawidłowe stężenia — zarówno niedobór, jak i nadmiar — mogą wiązać się ze zwiększonym ryzykiem zachorowania na nowotwory złośliwe.</p>

<h3>Dlaczego warto wykonać badanie onkopierwiastków właśnie we Wrocławiu?</h3>
<p>Mieszkańcy Wrocławia i okolicznych miast Dolnego Śląska — Oleśnicy, Oławy, Trzebnicy, Środy Śląskiej czy Kątów Wrocławskich — mogą skorzystać z <strong>badania onkopierwiastków we Wrocławiu</strong> bez konieczności dalekich podróży. Certyfikowane <strong>punkty pobrań onkopakietu we Wrocławiu</strong> — DOLMED i Centrum Medyczne Polmed — są doskonale skomunikowane z centrum miasta.</p>
...
<p>Proces wykonania <strong>badania onkopierwiastków we Wrocławiu</strong> jest prosty i nieinwazyjny. Pierwszy krok to zamówienie <strong>onkopakietu</strong> online na stronie onkopierwiastki.pl — wybierasz panel, placówkę we Wrocławiu i dokonujesz płatności. Następnie zgłaszasz się na pobranie krwi w wybranej placówce referencyjnej.</p>
...
<p>Podobne zależności zaobserwowano dla pozostałych pierwiastków — cynku, arsenu, miedzi, kadmu i ołowiu. Dlatego <strong>onkopakiet</strong> obejmuje wszystkie sześć jednocześnie — tylko pełny profil <strong>badania pierwiastków we krwi</strong> pozwala na rzetelną ocenę ryzyka nowotworowego.</p>

<h3>Gdzie przebadać się na onkopierwiastki we Wrocławiu — praktyczne informacje</h3>
<p>Certyfikowane <strong>placówki onkopakietu we Wrocławiu</strong> to Dolnośląskie Centrum Medyczne DOLMED S.A. przy ul. Legnickiej 40 oraz Centrum Medyczne Polmed przy ul. Grabiszyńskiej 208. Obie placówki pobierają krew pełną zgodnie z protokołem laboratorium i są czynne w dni robocze. Dojazd komunikacją miejską jest wygodny — w pobliżu obu placówek znajdują się przystanki tramwajowe i autobusowe.</p>

<p>Warto pamiętać, że <strong>badanie onkopierwiastków</strong> wymaga specjalnych próbówek i ścisłej procedury pobrania. Tylko w certyfikowanych placówkach referencyjnych masz pewność, że materiał zostanie pobrany i zabezpieczony prawidłowo. Standardowe laboratoria i <strong>punkty pobrań</strong> nie dysponują odpowiednim wyposażeniem do tego badania.</p>

<p>Alternatywnie, jeśli żadna placówka we Wrocławiu nie jest dla Ciebie dogodna, możesz zamówić <strong>onkopakiet</strong> online z dostawą zestawu pobraniowego kurierem. Szczegóły znajdziesz na stronie zamówienia.</p>

<h3>Badanie onkopierwiastków Wrocław — podsumowanie</h3>
<p>Mieszkańcy Wrocławia i Dolnego Śląska mają dostęp do innowacyjnego badania profilaktycznego, które na podstawie jednego pobrania krwi dostarcza informacji o stężeniach sześciu kluczowych pierwiastków śladowych. Wynik <strong>onkopakietu</strong> z indywidualnymi zaleceniami, oparty na spersonalizowanych normach dla polskiej populacji, może stanowić istotny element świadomej profilaktyki nowotworowej. <strong>Badanie onkopierwiastków Wrocław</strong> jest dostępne bez skierowania — zamów online i przyjdź na pobranie.</p>`,
  },
];
